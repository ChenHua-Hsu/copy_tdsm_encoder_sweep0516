...
def main(config=None):
    ...
    for filename in os.listdir(training_file_path):
        if fnmatch.fnmatch(filename, 'dataset_2_padded_nentry1033*.pt'):
...
#                trained_model_name = 'training_20240408_1350_output/ckpt_tmp_299.pth'
                trained_model_name = '/home/ken91021615/tdsm_encoder_sweep0516/training_result/training_20240701_1506_output/ckpt_tmp_199.pth'
                output_directory = generate(files_list_, load_filename=trained_model_name, device=device)
...
#            Converter_ = Convertor.Convertor(files_list_[0], 0.0, preprocessor=args.preprocessor)
            ...
            Converter_.digitize()
            Converter_.to_h5py(os.path.join(output_directory, 'Gen_r_width_new.h5'))
# #             # Convert Reference file: TODO: multifile management
            Converter_ = Convertor.Convertor(files_list_[0], 0.0, preprocessor=args.preprocessor)
            Converter_.invert(-99)
            Converter_.digitize()
#             Converter_.plot_re(r_gen,E_gen,z_gen)
            Converter_.to_h5py(os.path.join(output_directory, 'Reference_r_width_new.h5'))

   
            os.system('python util/evaluate_image_based_test.py -m all --output_dir {outdir} --input_file {Gen_file} --reference_file {Geant4_file} --dataset 2'.format(Gen_file = os.path.join(output_directory, 'Gen_r_width_new.h5'), Geant4_file = os.path.join(output_directory, 'Reference_r_width_new.h5'), outdir = os.path.join(output_directory, 'calo_score')))
